...
async function getMatches() {
  ...
  try {
    ...
    console.log(`Ladbrokes - Error getting matches ${error}`);
    return Promise.resolve([]);
  }
}
...
async function getEventCard(eventId) {
  ...
  try {
    ...
    console.log(`Ladbrokes - Error getting event card for ${eventId} : ${error}`);
    return Promise.resolve([]);
  }
}
...
async function getPlayerMarkets(eventName, marketType) {
  try {
    ...
    console.log(`Ladbrokes - Error getting player markets for ${eventName} : ${error}`);
    return Promise.resolve([]);
  }
}
